...
import __init_paths
from face_detect.retinaface_detection1 import RetinaFaceDetection
...
class FaceEnhancement(object):
    def __init__(self, base_dir='./', size=1024, model=None, use_sr=True, sr_model=None, channel_multiplier=2, narrow=1, key=None, device='cuda'):
    ...
    def mask_postprocess(self, mask, thres=18):
        mask[:thres, :] = 0; mask[-thres:, :] = 0
        mask[:, :thres] = 0; mask[:, -thres:] = 0
        mask = cv2.GaussianBlur(mask, (101, 101), 8)
...
if __name__=='__main__':
    ...
    parser.add_argument('--outdir', type=str, default='examples/outs', help='output folder')
    ...
    for n, file in enumerate(files[:]):
        ...
        cv2.imwrite(os.path.join(args.outdir, '.'.join(filename.split('.')[:-1])+'_1.jpg'), img)
        #cv2.imwrite(os.path.join(args.outdir, '.'.join(filename.split('.')[:-1])+'_Mask.png'), full_mask)
        
        #for m, (ef, of) in enumerate(zip(enhanced_faces, orig_faces)):
            #of = cv2.resize(of, ef.shape[:2])
            #cv2.imwrite(os.path.join(args.outdir, '.'.join(filename.split('.')[:-1])+'_face%02d'%m+'.jpg'), np.hstack((of, ef)))
            #cv2.imwrite(os.path.join(args.outdir, '.'.join(filename.split('.')[:-1])+'_face%02d'%m+'_1.jpg'), ef)
